...
@Component({
  selector: 'app-root',
  imports: [RouterOutlet, HeadBar, SearchBar, FooterComponent, CurrencyPipe],
...
export class App implements OnInit {
  ...
  toggleView(view: 'search' | 'favorites') {
    this.currentView = view;
  }

  // MÉTHODES POUR GÉRER LES FAVORIS
  getFavoriteCars(): Auto[] {
    return this.favoriteService.getFavorites();
  }

  removeFromFavorites(auto: Auto): void {
    this.favoriteService.toggleFavorite(auto);
    // Mettre à jour la propriété favorite dans la liste principale
    const index = this.autoList.findIndex(a => a.id === auto.id);
    if (index !== -1) {
      this.autoList[index].favorite = false;
    }
  }
  ...
  clearAllFavorites(): void {
    if (confirm('Voulez-vous vraiment supprimer tous vos favoris ?')) {
      ...
      // Réinitialiser tous les favoris dans autoList
      this.autoList.forEach(auto => auto.favorite = false);
    }
  }
  ...
  autoList: Auto[] = [
    ...
    },
    {
      ...
      photo: "GIULIA.webp",
      availability: 11,
      description: "The Alfa Romeo Giulia is a premium sports sedan renowned for its elegant design, dynamic performance, and advanced technology. It is available with a range of engines, including a 280-horsepower turbocharged gasoline engine and 160- ou 210-horsepower diesel options. The Giulia features a customizable 12.3-inch digital instrument cluster, an 8.8-inch touchscreen with navigation, and advanced driver assistance systems.",
      favorite: false
    }
  ];
}